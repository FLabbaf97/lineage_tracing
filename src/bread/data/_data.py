...
class Segmentation:
    ...
    def __post_init__(self):
        if self.data.ndim == 2:
            warnings.warn('Segmentation was given data with 2 dimensions, adding an empty dimension for time.')
        ...
        # calculate the average cell r_equive in pixels
        self.average_cell_diameter_pixel = self.calculate_average_cell_diameter_pixels()
    ...
    def birth_time(self, cell_id: int) -> int:
        """Returns the time index of the first frame where the cell appears
        ...
        -------
        int
            time index of the first frame where the cell appears
        """

        for idt in range(len(self)):
            if cell_id in self.cell_ids(idt):
                return idt

        raise ValueError(f'Cell {cell_id} not found in segmentation')
    ...
    def calculate_average_cell_diameter_pixels(self):
        ...
        return 2 * np.sqrt(average_cell_size / np.pi)
...
class Contour:
    ...
    def from_segmentation(seg: Segmentation, cell_id: int, time_id: int) -> 'Contour':
        ...
        if len(contours_cv) > 1:
            ...
            print(f'this happened for cell_id={cell_id}, time_id={time_id}')
...
class Ellipse:
    ...
    def from_contour(contour: Contour):
        try:
            ...
            print(contour.data)
            raise Contour.InvalidContourException(contour.data)
